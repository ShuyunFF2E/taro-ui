...
function getTooltipPositionInBody(tooltip, target, placement) {
  const { offsetWidth, offsetHeight } = tooltip; // tooltip本身宽高
  const { left, top, right, bottom, width, height } = getAbsPosition(target); // 元素绝对位置
...
export default class ToolView extends Component {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      this.setState(
        {
          style: getTooltipPositionInBody(tooltip, _target, placement),
          dir: getDirection(tooltip, _target, placement).join('-'),
        },
  ...
  render() {
    ...
    if (typeof content !== 'object') {
      return <View {...props} dangerouslySetInnerHTML={{ __html: content }} />;
    }
